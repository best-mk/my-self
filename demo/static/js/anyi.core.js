...
(function($){
	...
	$.ajaxPrefilter( function( options, originalOptions, jqXHR ) {
		var needToken = anyi.global.needToken(options.url);
		options.url = anyi.global.host(options.url);
		options.headers = options.headers || {};
		var sso_token = $.cookie("sso_token");
		if(needToken && sso_token && !options.headers["sso-token"]) {
			options.headers["sso-token"] = sso_token;
		}
	});
	...
	$(document).ajaxComplete(function(event, jqXhr, settings) {
		...
		if (sStatus === 'timeout' && settings.redirect !== false) {
			window.top.location.href = "/login.html?from=" + encodeURIComponent(location.href);
			return;
		}
		if (jqXhr.status === 401 && settings.redirect !== false) {
			window.top.location.href = "/login.html?from=" + encodeURIComponent(location.href);
			return;
		}
		//隐藏loading
...
anyi.global = (function () {
	...
	self.config = {
			"www.anyi-tech.com": "pro", "www.parallel.anyi-tech.com": "pre", "test.anyi-tech.com": "dev"
		}[location.host] || "dev";

	self.dev = {
		"core": "http://test.core.riskeys.com",
		"ocr": "http://test.ocr.riskeys.com",
		"anyi": "http://test.anyi-tech.com",
		"sso": "http://sso.test.anyi-tech.com",
		"appid": "wx830b6d5a4c135731"
	};

	self.pre = {
		"core": "http://lscore.riskeys.com",
		"ocr": "http://ocr.riskeys.com",
		"anyi": "http://www.parallel.anyi-tech.com",
		"sso": "http://sso.anyi-tech.com",
        "appid": "wx75c7390d500142b4"
	};

	self.pro  = {
		"core": "http://lscore.riskeys.com",
		"ocr": "http://ocr.riskeys.com",
		"anyi": "http://www.anyi-tech.com",
		"sso": "http://sso.anyi-tech.com",
        "appid": "wx75c7390d500142b4"
	};
	 ...
	 * @returns {boolean}
	 */
	self.needToken = function (url) {
		return /\/ws_sso\//.test(url) || /\/spb\//.test(url);
	};
	
	self.host = function (url) {
		if (/^\/ws_sso/.test(url)) {
			return self.getUrl("sso") + url;
		} else if (/^\/spb\//.test(url)) {
			return self.getUrl("anyi") + url;
		} else if(/^\/ws_common\/upload_file/.test(url)) {
			return self.getUrl("core") + url;
		}
		return url
	};
	return self;
...
anyi.common = (function(self) {
	...
	var loginTemplate = function (name) {
		var template = {
			"loggedIn": '<a class="pull-left login">登录</a> <a class="pull-left register">注册</a>',
			"notLoggedIn": "<a class='pull-left account' href='personal-center.html'></a><a class='pull-left sign-out'>退出</a>"
		};
		return template[name] || "";
	};

	var setLoginUrl = function ($login,redirect) {
		var from = encodeURIComponent(redirect || location.href);
		$login.find(".login").attr("href", "/login.html?from=" + from);
		$login.find(".register").attr("href", "/register.html?from=" + from);
	};
	...
	var setUserInfo = function () {
		...
		var redirect = $login.data("redirect");
		if(redirect && !token) {
			goLoginPage();
		} else if (anyi.sso && token) {
			var sso = new anyi.sso();
			var promise = redirect ? sso.userInfo() : sso.userInfo2();
			promise.done(function (data) {
				var $login = $("#nav-login");
				if (data.code === 1 && data.result) {
					var accountType = data.result.accountType;
					var userInfo = data.result.ssoUserInfo;
					$.session("anyi_user_info",data.result);
					var integrity = 0;
					for (var i = 0; i < accountType.length; i++) {
						var thisAccount = accountType[i];
						if (thisAccount.type === "账号" && thisAccount.status === "正常") {
							$login.html(loginTemplate("notLoggedIn"));
							$login.find(".account").text(thisAccount.account);
							$login.find(".sign-out").off("click.logout").on("click.logout", function () {
								var sso = new anyi.sso();
								sso.logout().done(function (data) {
									if (data.code === 1) {
										var $login = $("#nav-login");
										if ($login.data("redirect")) {
											location.href = "/index.html";
										} else {
											$login.html(loginTemplate("loggedIn"));
											setLoginUrl($login);
										}
									}
								}).always(function () {
			...
			setLoginUrl($login, "/personal-center.html");
		}
	};
	...
	self.tableLanguage = {
		...
		"emptyTable": "<img src='/img/no-policy.png'><p>暂无数据</p>",
		"zeroRecords": "没有匹配结果",
...
anyi.form = (function (self) {
	 ...
	 * @param format 格式化
     */
    self.bind = function(selector,model,format) {
        var $selector = $(selector);
        if ($selector.length > 0) {
            $selector.find("[data-field]").each(function (index, dom) {
                var nodeName = dom.nodeName;
